...
const TicketsController = {
  createTicket: async (req, res) => {
    ...
    try {
      ...
      const { Category, Sub_Category, Priority, Status, TDescribtion, userRate } = req.body;
  
      // Check for required fields
      if (!Category || !Sub_Category || !Priority || !Status || !TDescribtion || !userRate) {
        return res.status(400).json({ error: 'All fields are required' });
      }
  
      // Create a new Ticket object with the provided data and userId
